...
Status: visual direction approved on September 25, 2026 from mockups of the main screens; implementation not started. The plan runs inside [M3b.5](PHASE3B.md), before the test with three external users. It changes neither the protocol nor the relay, except optional package F1 (QR invitation), which needs its own format review.
...
Findings from September 25, 2026: `HistoryEventView` has no sender or time, and the receive path in `arveil-app` records events without the sender that `mls-rs` identifies through `sender_index`. `ConversationView` has no last message, last activity or unread count, and there is no read marker. The profile database has no schema versioning (`CREATE TABLE IF NOT EXISTS` only). Kit export or deferral lives only in panel memory. The last sync time exists only in the Dart conversation controller, which syncs every 10 seconds while open; that is acceptable as presentation state.
...
|---|---|---|---|
| A0 Profile schema versioning | M | — | `PRAGMA user_version`, ordered transactional migrations, typed rejection of a future version without touching data; existing unversioned databases (builds up to `0.1.0+11`) are version 0 |
| A1 Sender and time in history | M | A0 | Store the sender device and identity from the MLS member credential; `HistoryEventView` gains sender, label, own flag and `created_at` (local recording time, not send time); new archive version, importer still accepts version 1 |
