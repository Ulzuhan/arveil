...
- **Color:** warm paper ground `#F4F1EA` / `#0E1413`, ink `#16211F` / `#E6ECEA`, brand accent `#245B51` / `#8FD0C0`, own bubble `#DDEAE4` / `#1A423B` (darkened in D1 so muted text on a selected row keeps 4.5:1), amber attention `#F6E7CC`+`#6B4108` / `#3A2A12`+`#F2C98A`, danger `#A2382B` / `#F2A59B`. Five avatar tones and four sender-name colors per theme, assigned deterministically from the identity ID. On September 25, 2026 every text/background pair measured at least 5.05:1; B1 turns this into an automated test.
...
| C4 Welcome and enrollment (implemented) | M | B1, B2 | Three entry points, step-by-step invitation flow, pairing and restore, kit with "Later" and visible risk |
