...
| Canjear una invitación dos veces | El relay registra lo que produjo un canje —token, identidad y credencial— dentro de la misma transacción que consume el uso, y responde a la repetición de esa terna exacta con el resultado que registró, no con un conflicto. Otra credencial para la misma identidad sigue siendo conflicto: la igualdad de un hash no es autorización. Una repetición no consume otro uso, y una base escrita por un relay más nuevo se rechaza antes de modificar nada. La creación de buzón todavía no es idempotente, así que el alta completa aún no es reanudable. |
